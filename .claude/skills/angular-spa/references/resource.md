<!-- Adapted from the official Angular agent skill (angular-developer, MIT, Copyright 2026 Google LLC). -->
...
- `httpResource` vive SOLO en services (`GameService.gameResource(() => id())`) y se llama
  desde el inicializador de un campo del componente (contexto de inyección).
- La URL function devuelve `undefined` para "todavía no pedir".
- Después de un comando (POST), actualizar localmente con la respuesta
  (`this.game.set(result.game)`) o `reload()`.
- Errores: `resource.error()` → `extractErrorMessage(...)` en un `computed()`.
